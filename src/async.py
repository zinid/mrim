...
def send(obj, msg):
    queue.put_nowait((weakref.ref(obj), msg))
...
##        self.__dict__['socket'] = sock
    ...
    def set_timer(self, pause, msg):
        ...
        bisect.insort(timers, (stoptime, tref, self))
        self._trefs[tref] = (stoptime, msg)
        return tref

    def cancel_timer(self, tref):
        try:
            stoptime = self._trefs.pop(tref)[0]
            timers.remove((stoptime, tref, self))
        except (KeyError, ValueError):
            pass

    def stop_timers(self):
        while self._trefs:
            tref, val = self._trefs.popitem()
            try:
                timers.remove((val[0], tref, self))
            except ValueError:
                pass
    ...
    def handle_timer_event(self, tref):
        msg = self._trefs.pop(tref)[1]
        self.handle_timer(tref, msg)

    def handle_queue_event(self, msg):
        self.handle_queue(msg)
    ...
    def handle_queue(self, msg):
        self.log_info('unhandled queue event', 'warning')
...
class dispatcher_with_send(dispatcher):
    ...
    def initiate_send(self):
        num_sent = 0
        num_sent = dispatcher.async_send(self, self.out_buffer[:512])
...
def process_timers(maxtimeout):
    while timers:
        stoptime, tref, obj = timers.pop(0)
        now = ticks()
        if now >= stoptime:
            try:
            ...
            timers.insert(0, (stoptime, tref, obj))
...
def process_queue():
    while 1:
        try:
            ref, msg = queue.get_nowait()
        except Queue.Empty:
            break
        obj = ref()
        if obj:
            try:
                obj.handle_queue_event(msg)
            except ExitNow:
                raise
            except:
                obj.handle_error()
